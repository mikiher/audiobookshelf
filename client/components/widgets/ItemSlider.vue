...
export default {
  ...
  computed: {
    booksPerPage() {
      return Math.floor(this.clientWidth / this.cardWidth)
    },
    isSelectionMode() {
    ...
    editFunc() {
      switch (this.type) {
        case 'episode': return this.editEpisode
        case 'authors': return this.editAuthor
        default:        return this.editItem
      }
    }
  },
  methods: {
    ...
    setCardWidth() {
      let itemRef = this.$refs.item
      if (itemRef && itemRef.length > 0) {
        const { clientWidth } = itemRef[0]
